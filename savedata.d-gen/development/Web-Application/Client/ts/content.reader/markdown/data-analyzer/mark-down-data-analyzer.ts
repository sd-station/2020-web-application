...
declare global {
    interface String {
        TextAfter: (str: string) => string;
    }
}

String.prototype.TextAfter = function (code: string): string {

    if (code == null || code.length == 0 || this.indexOf(code) < 0) return this as string;
    return this.substring(this.indexOf(code) + code.length) as string
}
export class MarkDownDataAnalyzer {

    Edata: IFieldFile = new IFieldFile();

    GetFromMD(linesR: string[]): MarkDownDataAnalyzer {
        ...
        linesR.forEach((lnraw, i) => {
            ...
            let ln = lnraw;
            while (ln.indexOf("&nbsp;") >= 0) ln = ln.replaceAll("&nbsp;", " ");
            ...
            if (tempItem.space > 0) {
                ...
                if (possibleparents.length > 0) tempItem.parent = possibleparents[possibleparents.length - 1].id;
            }
            ...
            normaltags.forEach(ckey => {
                if (ln.trim().startsWith(ckey)) {
                    ...
                    if (inputtag.indexOf(ckey) >= 0) tempItem.mode = "field";
            ...
            if (tempItem.html.tag.length > 0) return;
            ...
            if (ln.trim().startsWith("input")) {
                ...
                if (this.Edata.name.length > 0) tempItem.html.id = this.Edata.name.toLowerCase().replace(/\ /g, "-") + "-" + tempItem.html.id;
                this.Edata.items.push(tempItem);
    ...
    ExtractData(lnc: IFieldRow, arg1: string) {
        var ops = arg1.split(" ");
        ...
        ops.forEach(u => {
            ...
            if (ActiveAttachment) {
                ...
                if (u.endsWith("\"")) ActiveAttachment = false;
                return;
            }

            if (u.startsWith("#") && u.length > 1) { lnc.html.id = u.substring(1); return; }
            if (u.startsWith(".") && u.length > 1) { lnc.html.class.push(u.substring(1)); return; }
            if (u.indexOf("=") > 0) {
