...
export class HtmlCodeProvider {
    ...
    GetHtmlData(parent: IFieldRow | null): string {
     ...
     * @param El 
     */
    private ToHtml(El: IFieldRow, parenttag: string = ""): string {
       ...
       // Parameters.push(`data-identify="I${El.id}-P${El.parent}-S${El.space}"`)
        if (El.html.id.length > 0) { Parameters.push(`id="${El.html.id}"`) }
        if (El.html.class.length > 0) { Parameters.push(`class="${El.html.class.join(" ")}"`) }
        ...
        SingleTags.forEach(STAG => {
            if (El.html.tag == STAG) res = `<${STAG} ${Attr}>${El.html.content}</${STAG}>`
        })
        ...
        ContainerTag.forEach(STAG => {
            if (El.html.tag == STAG) res = `<${STAG} ${Attr}>${El.html.content}${content}</${STAG}>`
        })
        if (res.length > 0) return res;
        ...
        SingleTag.forEach(STAG => {
            if (El.html.tag == STAG) res = `<${STAG} ${Attr} ${El.html.content}${content}>`
        })
        if (res.length > 0) return res;


        if (El.html.tag == "label") return `<label ${Attr}><span>${El.html.content}</span>${content}</label>`
