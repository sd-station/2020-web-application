import { CHtmlInfo } from "../lib/i-html-info";
...
export class DataStandard {
    ...
    MakeName(HI: CHtmlInfo) {
        if (HI.id.length == 0) return "UnNamed";

 
        var inpg = HI.id;

        if (HI.tag == "form") this.FormNameSpace = inpg;
        if (HI.tag == "input" && HI.id.startsWith(this.FormNameSpace)) {
            inpg = inpg.substring(this.FormNameSpace.length);
            if (inpg.startsWith("-")) inpg = inpg.substring(1);
        }
    ...
    FixAllData() {
        window.EData.items.filter(k => ["input", "textarea", "select"].includes(k.html.tag)).forEach(row => {
            if (row.name.length == 0) row.name = this.MakeName(row.html).toLowerCase();
        });
